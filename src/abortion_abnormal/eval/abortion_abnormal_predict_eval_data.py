...
class AbortionAbnormalPredictEvalData_v3():
    ...
    def get_special_sample(self, data, period, prev_status, next_status):
        ...
        if prev_status == 0 and next_status == 1: # 获取从正常到异常样本
            for pigfarm, group_data in grouped:
                # 按时间排序
                sorted_data = group_data.sort_values('stats_dt')
                ...
                for i in range(len(sorted_data) - 1):
                    if sorted_data.iloc[i][self.truth_column.format(period)] == prev_status and sorted_data.iloc[i+1][self.truth_column.format(period)] == next_status:
                        ...
                        farm_data = sorted_data.copy()
                        t_minus_1 = t - pd.Timedelta(days=1)
                        t_minus_2 = t - pd.Timedelta(days=2)
                        t_minus_3 = t - pd.Timedelta(days=3)
                        t_plus_6 = t + pd.Timedelta(days=6)

                        # 筛选时间范围内的样本
                        time_range_samples = farm_data[(farm_data['stats_dt'] >= t_minus_3) &
                                                        (farm_data['stats_dt'] <= t_plus_6)]

                        # 判断是否符合连续性，即两天的样本是相同的
                        t_minus_1_sample = time_range_samples[time_range_samples['stats_dt'] == t_minus_1]
                        t_minus_2_sample = time_range_samples[time_range_samples['stats_dt'] == t_minus_2]
                        t_minus_3_sample = time_range_samples[time_range_samples['stats_dt'] == t_minus_3]
                        t_sample = time_range_samples[time_range_samples['stats_dt'] == t]

                        # 检查样本
                        if not t_minus_1_sample.empty:
                            # 获取 abort_{period} 值
                            t_minus_1_abort = t_minus_1_sample.iloc[0][self.truth_column.format(period)]
                            # 如果两天的 abort_{period} 值不不为0，删除 t_minus_1 的数据
                            if t_minus_1_abort != 0:
                                time_range_samples = time_range_samples[time_range_samples['stats_dt'] != t_minus_1]
                        if not t_minus_2_sample.empty:
                            t_minus_2_abort = t_minus_2_sample.iloc[0][self.truth_column.format(period)]
                            if t_minus_2_abort != 0:
                                time_range_samples = time_range_samples[time_range_samples['stats_dt'] != t_minus_2]
                        if not t_minus_3_sample.empty:
                            t_minus_3_abort = t_minus_3_sample.iloc[0][self.truth_column.format(period)]
                            if t_minus_3_abort != 0:
                                time_range_samples = time_range_samples[time_range_samples['stats_dt'] != t_minus_3]

                        # 添加到特殊样本列表
                        special_samples.append(time_range_samples)

        elif prev_status == 1 and next_status == 0: # 获取从异常到正常的样本
            for pigfarm, group_data in grouped:
                # 按时间排序
                sorted_data = group_data.sort_values('stats_dt')
                ...
                for i in range(len(sorted_data) - 1):
                    if sorted_data.iloc[i][self.truth_column.format(period)] == prev_status and sorted_data.iloc[i+1][self.truth_column.format(period)] == next_status:
                        ...
                        # 找出t+1到t+2的所有样本
                        farm_data = sorted_data.copy()
                        t_minus_5 = t - pd.Timedelta(days=5)
                        t_plus_1 = t + pd.Timedelta(days=1)
                        t_plus_2 = t + pd.Timedelta(days=2)
                        t_plus_3 = t + pd.Timedelta(days=3)
                        t_plus_4 = t + pd.Timedelta(days=4)

                        # 筛选时间范围内的样本
                        time_range_samples = farm_data[(farm_data['stats_dt'] >= t_minus_5) &
                                                        (farm_data['stats_dt'] <= t_plus_4)]

                        # 判断是否符合连续性，即两天的样本是相同的
                        t_plus_2_sample = time_range_samples[time_range_samples['stats_dt'] == t_plus_2]
                        t_plus_3_sample = time_range_samples[time_range_samples['stats_dt'] == t_plus_3]
                        t_plus_4_sample = time_range_samples[time_range_samples['stats_dt'] == t_plus_4]

                        # 如果 t+1 和 t+2 的样本存在，检查它们的 abort_{period} 值
                        if not t_plus_2_sample.empty:
                            t_plus_2_abort = t_plus_2_sample.iloc[0][self.truth_column.format(period)]
                            # 如果两天的 abort_{period} 值不同，删除 t+2 的数据
                            if t_plus_2_abort != 0:
                                time_range_samples = time_range_samples[time_range_samples['stats_dt'] != t_plus_2]
                        if not t_plus_3_sample.empty:
                            t_plus_3_abort = t_plus_3_sample.iloc[0][self.truth_column.format(period)]
                            if t_plus_3_abort != 0:
                                time_range_samples = time_range_samples[time_range_samples['stats_dt'] != t_plus_3]
                        if not t_plus_4_sample.empty:
                            t_plus_4_abort = t_plus_4_sample.iloc[0][self.truth_column.format(period)]
                            if t_plus_4_abort != 0:
                                time_range_samples = time_range_samples[time_range_samples['stats_dt'] != t_plus_4]
            ...
            raise ValueError("prev_status 和 next_status 的组合不合法")
