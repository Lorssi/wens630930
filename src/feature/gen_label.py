...
class LabelGenerator:
    ...
    def has_risk_4_class_period_generate_label_alter(self):
        ...
        farm_count = 0
        # 处理每个猪场的数据
        for farm_id, farm_data in farm_dict.items():
            farm_count += 1
            # 创建猪场ID和日期到原始索引的映射
            date_to_idx = {}
            ...
            # 更新处理进度
            processed_count += len(farm_data)
            # 每处理10个猪场或者总进度达到整数百分比时更新一次
            if farm_count % 10 == 0 or processed_count % (total_count // 100 + 1) == 0 or processed_count == total_count:
                print(f"处理进度: {processed_count}/{total_count} ({processed_count/total_count*100:.1f}%), 已处理{farm_count}/{len(farm_dict)}个猪场")
