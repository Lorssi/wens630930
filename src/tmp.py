...
if __name__ == "__main__":
    ...
    # 生成lable
    df = pd.read_csv("change_days_to_risk.csv")
    label_generator = LabelGenerator(
        feature_data=df,
        running_dt="2024-12-31",
        interval_days=60
    )
    logger.info("开始生成标签...")
    X, y = label_generator.has_risk_period_generate_multi_label_alter_nodays_expand14() 
    result = pd.concat([X,y], axis=1)
    result.to_csv("change_days_to_risk_label.csv", index=False)
