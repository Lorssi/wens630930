...
class MultiTaskDataset(Dataset):
    ...
    """
    def __init__(self, df: pd.DataFrame, risk_label: list, days_label: list):
        self.df = df

        self.risk_label_list = risk_label  # 风险标签列表
        self.days_label_list = days_label  # 风险天数标签列表
    ...
    def __getitem__(self, idx):
        ...
        feature = row.drop(self.risk_label_list + self.days_label_list).values
        has_risk_label = row[self.risk_label_list].values
        days_risk_label = row[self.days_label_list].values
            
        return feature, has_risk_label, days_risk_label
...
def calculate_all_task_weights(train_y, num_classes=8, method='inverse'):
    ...
    for i, label_col in enumerate(days_label_list):
        task_name = f"days_task_{periods[i][0]}_{periods[i][1]}"
...
def _collate_fn(batch):
    ...
    for feature, risk_label, days_label in batch:
        # 添加特征
        features_list.append(feature)
        
        # 多标签任务（前三个）- 需要作为一个向量
        risk_label_list.append(risk_label)
        days_label_list.append(days_label)
        
    
    # 转换为张量
    features_tensor = torch.tensor(np.array(features_list), dtype=torch.float32)
    risk_label_tensor = torch.tensor(np.array(risk_label_list), dtype=torch.float32)  # 多标签用float
    days_label_tensor = torch.tensor(np.array(days_label_list), dtype=torch.long)
    
    # 返回特征和所有标签
    return features_tensor, risk_label_tensor, days_label_tensor
...
def train_model(model, train_loader, val_loader, criterion, optimizer, num_epochs, device, early_stopping=None):
    ...
    risk_criterion, days_criterion_1_7, days_criterion_8_14, days_criterion_15_21 = criterion
    
    for epoch in range(num_epochs):
        model.train()
        train_loss = 0.0
        for batch_idx, (features, risk_label, days_label) in enumerate(train_loader):
            features, risk_label, days_label = features.to(device), risk_label.to(device), days_label.to(device)  # 将特征和标签移至目标设备
            # label

            optimizer.zero_grad()
            risk_outputs, days_output_1_7, days_output_8_14, days_output_15_21 = model(features)
            risk_loss = risk_criterion(risk_outputs, risk_label)
            days_loss = days_criterion_1_7(days_output_1_7, days_label[:, 0]) + days_criterion_8_14(days_output_8_14, days_label[:, 1]) + days_criterion_15_21(days_output_15_21, days_label[:, 2]) 

            loss = risk_loss + 0.5 * days_loss
            loss.backward()
...
def eval(model, val_loader, criterion, device):
    ...
    with torch.no_grad():
        for features, risk_label, days_label in val_loader:
            # 获取批次数据并移至目标设备
            features, risk_label, days_label = features.to(device), risk_label.to(device), days_label.to(device)  # 将特征和标签移至目标设备
            
            # 前向传播
            risk_outputs, days_output_1_7, days_output_8_14, days_output_15_21 = model(features)
            loss = risk_criterion(risk_outputs, risk_label) + days_criterion_1_7(days_output_1_7, days_label[:, 0]) + days_criterion_8_14(days_output_8_14, days_label[:, 1]) + days_criterion_15_21(days_output_15_21, days_label[:, 2])
            val_loss += loss.item()
            
            # 获取预测结果 - 使用sigmoid而非softmax
            probs = torch.sigmoid(risk_outputs)
            preds = (probs > 0.5).float()  # 二值化，阈值为0.5
...
if __name__ == "__main__":
    ...
    logger.info("开始生成标签...")
    X, y = label_generator.has_risk_period_generate_multi_label_alter_risk_days()
    ...
    has_risk_label_list = [ColumnsConfig.HAS_RISK_4_CLASS_PRE.format(left, right) for left, right in periods]
    days_label_list = [ColumnsConfig.DAYS_RISK_8_CLASS_PRE.format(left, right) for left, right in periods]
    train_dataset = MultiTaskDataset(train_df, risk_label=has_risk_label_list, days_label=days_label_list)
    val_dataset = MultiTaskDataset(val_df, risk_label=has_risk_label_list, days_label=days_label_list)
    ...
    risk_criterion = nn.BCEWithLogitsLoss()  # 假设是回归任务，使用均方误差
    days_criterion_1_7 = FocalLoss(alpha=task_weights['days_task_1_7'], gamma=2.0)
    days_criterion_8_14 = FocalLoss(alpha=task_weights['days_task_8_14'], gamma=2.0)
    days_criterion_15_21 = FocalLoss(alpha=task_weights['days_task_15_21'], gamma=2.0)
    criterion = (risk_criterion, days_criterion_1_7, days_criterion_8_14, days_criterion_15_21)  # 损失函数
