...
class RiskTaskHead(nn.Module):

    def __init__(self, input_dim = 8, output_dim = 3):
        super().__init__()
        self.task_head = nn.Linear(input_dim, output_dim)

    def forward(self, x):
        output = self.task_head(x)
        return output
    
class DaysTaskHead(nn.Module):

    def __init__(self, input_dim = 8, output_dim = 3):
        super().__init__()
        self.task_head_1_7 = nn.Linear(input_dim, output_dim)
        self.task_head_8_14 = nn.Linear(input_dim, output_dim)
        self.task_head_15_21 = nn.Linear(input_dim, output_dim)

    def forward(self, x):
        output_1_7 = self.task_head_1_7(x)
        output_8_14 = self.task_head_8_14(x)
        output_15_21 = self.task_head_15_21(x)
        return output_1_7, output_8_14, output_15_21
...
class RiskDaysCombinedModel(nn.Module):
    """
    ...
    def __init__(self, params: dict): 
        super(RiskDaysCombinedModel, self).__init__()
        ...
        self.risk_task_head = RiskTaskHead(input_dim=8, output_dim=3)
        self.days_task_head = DaysTaskHead(input_dim=8, output_dim=8)
    ...
    def forward(self, x):
        ...
        # 输入到MLP
        feature = self.mlp(interaction_output)

        risk_output = self.risk_task_head(feature)
        days_output_1_7, days_output_8_14, days_output_15_21 = self.days_task_head(feature)
        
        return risk_output, days_output_1_7, days_output_8_14, days_output_15_21